...
class Attention(nn.Module):
    def __init__(
        ...
        self.pos_embeds = SinusoidalPosEmb(dim)
    ...
    def forward(
        ...
        if framerate is not None:
            x = x + self.pos_embeds(torch.from_numpy(np.linspace(0.0, float(x.shape[1]) / framerate, num=x.shape[1], endpoint=False)).float()).to(dtype=x.dtype, device=x.device)

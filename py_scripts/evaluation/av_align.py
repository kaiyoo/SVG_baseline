"""
Based on the official implementation of TempoToken (MIT license)
...
def calc_intersection_over_union(audio_peaks, video_peaks, fps):
    ...
    for video_peak in video_peaks:
        for audio_peak in audio_peaks:
            if audio_peak - 1 / fps < video_peak < audio_peak + 1 / fps:
                intersection_length += 1
                break
    p2 = intersection_length / len(video_peaks) if len(video_peaks) > 0 else 0
    return p1 * p2 / (p1 + p2 - p1 * p2) if p1 > 0 or p2 > 0 else 0.0
...
if __name__ == "__main__":
    ...
    files = [file for file in glob.glob(os.path.join(args.audio_dir, '*.wav'))]
    score = 0
    cnt = 0
